...
本技能接入导演中心的[闭环执行协议](../short-drama-director/SKILL.md#闭环执行协议)。本职责的闭环核验至少覆盖：每镜原文或授权故事稿来源、旁白与对白边界、逐段词头资产筛选、参考图绑定、人物实际位置及入画／排除范围、姿态与受力、道具交接、段间结束快照、轴线与切镜、时长、格式和工具字段。发现遗漏、穿帮或模型风险时，在不改变文本剧情的范围内进行最小修正并回查相邻镜头、段落、资产和检查表；缺少必要状态参考时不得标记可投产。文本审查通过只标记“可测试”，视频结果须经导演实测确认后才标记“最终通过”；交付时记录实际模块、修正轮次、未解决项和状态。
