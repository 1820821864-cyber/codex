...
19. 后文信息防泄露：通过
20. 每个生成段均携带单一完整的本段对应词头、逐镜原文映射与参考一致性：通过
...
34. 每段词头保持统一视觉基线，并完整覆盖本段全部实际人物、场景和道具，不引用本段未出现资产：通过
35. 无道具生成段省略道具状态栏：通过
36. 无人物生成段省略人物状态栏：通过
...
- 任何原文遗漏或改写、旁白进入模型、旁白与对白重叠、单段超过9镜、分段超出4–30秒、任一生成段缺少完整本段词头、词头遗漏本段实际资产、词头引用本段未出现资产、用“同上”代替词头、无依据状态跳变、非硬切过渡、参考外观被文字覆盖、缺图导致任务暂停、泛化环境底声、背景音乐、可读文字交给模型生成，均为失败并先修正。
